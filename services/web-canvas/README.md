...
- Files, classes, functions, methods, variables, external packages, and unresolved nodes visible by default. Derived directory, package, and module groups are off by default but independently selectable. Relationships on hidden module/package endpoints remain visible at their enclosing file boundaries, with counts and source locations preserved. Tests are excluded by default.
...
- Search/command palette, symbol tabs, bookmarks, local saved views, fit/reset, JSON/PNG export, responsive side panels, and keyboard navigation. Selection reveals the exact entity even in large collapsed files, enabling its kind and switching to Repository when outside Entry Focus. File labels show relative paths; module labels show qualified names. Container inspection includes contained dependencies and owner/member navigation. Repository opens by default; explicit Entry Focus uses the analyzer's reachability including imported-module initialization. If no entry is detected, Entry Focus explains why it is disabled.
